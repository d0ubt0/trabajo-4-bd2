...
\echo Consulta 1: doble NOT EXISTS
...
\echo Consulta 2: firma del conjunto con STRING_AGG
WITH conjuntos AS (
  SELECT p.nit, p.nombre,
         COALESCE(
           STRING_AGG(x.codigoProducto::TEXT, ',' ORDER BY x.codigoProducto),
           'SIN_PRODUCTOS'
         ) AS firma
  FROM Prov p
...
\echo Consulta 3: conteos y productos comunes
...
\echo Consulta 4: operadores de conjuntos EXCEPT
SELECT p1.nit AS Nitprov1, p1.nombre AS NombreProv1,
       p2.nit AS Nitprov2, p2.nombre AS NombreProv2
FROM Prov p1
JOIN Prov p2 ON p1.nit < p2.nit
