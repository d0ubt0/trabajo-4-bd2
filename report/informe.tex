...
5.000 / 50.000 & Q1 - NOT EXISTS & 165K & 22.24s & Costo 165K, MERGE JOIN + INDEX RANGE SCAN, 12.5M filas \\ \hline
5.000 / 50.000 & Q2 - LISTAGG & 261 & 0.05s & Sigue estable, TABLA TEMPORAL + HASH JOIN \\ \hline
5.000 / 50.000 & Q3 - conteos & 98 & 84.02s & NESTED LOOPS OUTER masivo, 124M filas intermedias, 43.500 lecturas físicas \\ \hline
5.000 / 50.000 & Q4 - MINUS & 4486 & 40.24s & 12.497.500 pares, 29M de LIOs, INDEX RANGE SCAN intensivo \\ \hline
\end{tabular}%
}
\end{table}
...
5.000 / 50.000 & Q1 & 153,069.38 & 13.57 & 0.0000 \\ \hline
5.000 / 50.000 & Q2 & 0.74 & 13.57 & 0.0000 \\ \hline
5.000 / 50.000 & Q3 & 34,195.10 & 13.57 & 0.0067 \\ \hline
5.000 / 50.000 & Q4 & 153,430.73 & 13.57 & 0.0000 \\ \hline
\end{tabular}%
}
\end{table}
...
5.000 / 50.000 & Q1 & 3.537 ms & 133.063 & 0 & Hash Anti Join + Nested Loop, 12.5M filas, JIT habilitado \\ \hline
5.000 / 50.000 & Q2 & 30.146 ms & 385 & 0 & Merge Join, plan eficiente \\ \hline
5.000 / 50.000 & Q3 & 21.168 ms & 54.310 & temp: 72644 & Hash Join masivo, 257 batches, 16MB en disco \\ \hline
5.000 / 50.000 & Q4 & 80.710 ms & 50.550.152 & 0 & 50.5M buffers, 12.5M iteraciones HashSetOp Except \\ \hline
\end{tabular}%
}
\end{table}
...
La consulta más rápida fue Q2 (\texttt{LISTAGG} / \texttt{STRING\_AGG}). Se mantuvo muy rápida (30 ms en Postgres y 0.05s en Oracle incluso con 5.000 proveedores) a pesar del crecimiento de la muestra.
La consulta más lenta fue Q4 (\texttt{MINUS} / \texttt{EXCEPT}), llegando a 80.71s en PostgreSQL con 5.000 proveedores. En Oracle alcanzó 40.24s para la misma muestra.

\subsubsection{Lecturas lógicas y Planes de Ejecución}
La enorme diferencia de rendimiento en Q4 (donde PostgreSQL procesó 50.5M de \textit{shared hits} con 5.000 proveedores vs Oracle con 29.1M \textit{LIOs}) radica en el plan de ejecución:
\begin{itemize}
...
\begin{enumerate}
    \item \textbf{La consulta más eficiente} fue Q2 (Firma con agregación de strings). Transformar un problema de conjuntos relacionales a un problema escalar evita el crecimiento cuadrático. Incluso con 5.000 proveedores se mantuvo por debajo de 0.05s en Oracle y 31 ms en PostgreSQL. Sin embargo, tiene la limitación de la longitud de bytes en Oracle.
    \item \textbf{Operadores de conjuntos (MINUS / EXCEPT)}: Fueron los enfoques más intuitivos, pero los de peor rendimiento. En PostgreSQL con 5.000 proveedores alcanzó 80.71s debido a la ejecución anidada del EXCEPT.
    ...
    \item \textbf{Oracle vs PostgreSQL}: Oracle supo aprovechar sus índices por defecto de manera más eficiente en los operadores \texttt{MINUS} (40.24s vs 80.71s en la muestra más grande), mientras que PostgreSQL fue superior en la manipulación rápida de \texttt{STRING\_AGG}.
\end{enumerate}
